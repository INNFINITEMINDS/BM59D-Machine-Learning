...
line([-13.345 -13.345],Boundary,'Color','k','LineStyle','--');
line([5.315 5.315],Boundary,'Color','k','LineStyle','--');
xlim([-25 17])
xlabel('x')
ylabel('R(\alpha_i|x)')
title('Class Risks with \lambda_1_2=0.5 and \lambda_2_1=1')
legend([p1 p2], 'R_1','R_2')

%% DECISION RULES and CONFUSION MATRICES
% For minimum risk, C_1   -13.35<x<5.32
%                   C_2   else

TrClassification=zeros(125,1);
ValClassification=zeros(125,1);
for i=1:125
    if xtr_classification(i,1)>-13.35 && xtr_classification(i,1)<5.32
        TrClassification(i)=1;
    else
        TrClassification(i)=2;
    end
    
    if xval_classification(i,1)>-13.35 && xval_classification(i,1)<5.32
